...
//		By allItems=By.cssSelector("#number-menu div");
	...
	public void TC04_Angula() {
		...
		selectItemInDropDownList(parent, allItems, "Football");
		// sleepInSecond(5);
		Assert.assertEquals(
	...
	public void TC06_Kenko() {
		...
		Assert.assertTrue(
				explicitWait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("kd-loader"))));
		Assert.assertTrue(explicitWait
				.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("span.k-i-loading"))));
		selectItemInDropDownList(By.cssSelector("span[aria-owns='categories_listbox']"),
				By.cssSelector("span.k-state-default h3"), "Produce");

		Assert.assertTrue(explicitWait
				.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("span.k-i-loading"))));
		selectItemInDropDownList(By.cssSelector("span[aria-owns='products_listbox']"),
				By.cssSelector("#products_listbox>li"), "Longlife Tofu");

		Assert.assertTrue(explicitWait
				.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("span.k-i-loading"))));
		selectItemInDropDownList(By.cssSelector("span[aria-owns='shipTo_listbox']"),
				By.cssSelector("#shipTo_listbox li"), "Kirchgasse 6");
	}
